...
$ julia -i test.jl
```
コマンド名に続いて引数を指定すると、julia から配列変数 ARGS によって参照することができます[^1]。
[^1]: 実行終了後に REPL で参照できる変数は global 変数に限られるので注意が必要です。例えば、関数や for 文の中で定義した変数は基本的に参照できません。デバッグ目的であれば、一時的に ```global``` を指定するとよいでしょう。
